$(document).ready(function(){
    ...
    // bago nga codes hereeee :)

    var usernameValid = false;

    $("#chati-login-form").on("submit", function(e){
        e.preventDefault();
        ...
        const response_tag = $("#chati-login-form-response");
        const form_tag = $(this);

        if(!usernameValid){
            param = { url: `${BASE_URL}login/auth`, data: form_data };
        }else{
            param = { url: `${BASE_URL}login/verifyPassword`, data: form_data };
        }
        ...
        sendAjax(param, function(response){
            ...
            if(response.success){
                ...
                if(!usernameValid){
                    $(`input[name='username']`).addClass("valid-username").prev().fadeOut();
                }

                usernameValid = true;

                $("#password-field").fadeIn();
                form_tag.find(`input[name='username']`).prop('readonly', true);
                // window.location.href = `${BASE_URL}login/index`;
            }else{
                response_tag.html(response.message);
            }
            console.log(response);
        });
    });

    $("#chati-register-form").on("submit", function(e){
        e.preventDefault();

        const form_data = new FormData(this);

        const param = { url: `${BASE_URL}register/create`, data: form_data };

        const response_tag = $("#chati-register-form-response");
        ...
        sendAjax(param, function(response){
            ...
            if(response.success){
                ...
                response_tag.addClass('text-danger');
            }
    ...
    $("#chati-message-form").on("submit", function(e){
        e.preventDefault();

        const formData = new FormData(this);

        const paramData = {url:`${BASE_URL}world-chat-send`, data:formData};

        sendAjax(paramData, function(response){
    ...
    var pusher = new Pusher('00b0be9d12434bc5b265', {
        cluster: 'ap1'
    });

    var channel = pusher.subscribe('chati-channel');
        channel.bind('chati-event', function(data) {
    ...
    function loadMessage(){
        ...
        const paramData = {url:`${BASE_URL}world-chat-get`, data:formData};
        ...
        sendAjax(paramData, function(response){
            ...
            $.each(response, function(k, v){
                html += `<div class="message ${(localName==v.Message.sender_name)?'sent':'received'}">
                <div class="message-info">
                    <span class="display-name">${(localName==v.Message.sender_name)?'You':v.Message.sender_name}</span>
                </div>
                <p>${v.Message.message_text}</p>
            </div>`;
            });
    ...
    $("#set-display-name").on("click", function(){
        const display_name = $(`input[name='sender_name']`).val();
        localStorage.setItem('display-name', display_name);

        alert(`${display_name} has been set as your display name`);

        location.reload();
    });

    const localName = localStorage.getItem('display-name');

    if(localName){
        $("#display-name").val(localName);
    }
